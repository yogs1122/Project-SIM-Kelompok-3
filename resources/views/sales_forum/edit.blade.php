<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-gray-800">Edit Post Penjualan</h2>
    </x-slot>

    <div class="py-6 px-6">
        <div class="max-w-2xl mx-auto bg-white shadow-lg rounded-xl p-8">
            <form action="{{ route('sales_forum.update', $salesForum) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PATCH')
                ...
                <div>
                    ...
                    <input 
                        ...
                        id="title" 
                        name="title" 
                        value="{{ old('title', $salesForum->title) }}"
                ...
                <div>
                    ...
                    >
                        @foreach (['umum' => 'Umum', 'produk' => 'Produk', 'layanan' => 'Layanan', 'lowongan' => 'Lowongan Kerja'] as $val => $label)
                            <option value="{{ $val }}" {{ old('category', $salesForum->category) === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('category')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea 
                        ...
                        class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-green-500 transition"
                        required
                    >{{ old('description', $salesForum->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center">
                        ...
                        <input 
                            ...
                            id="price" 
                            name="price" 
                            value="{{ old('price', $salesForum->price) }}"
                ...
                <div>
                    ...
                    @if ($salesForum->image)
                        <div class="mb-4 bg-gray-50 p-4 rounded-lg">
                            ...
                            <img src="{{ asset('storage/' . $salesForum->image) }}" alt="{{ $salesForum->title }}" class="h-32 object-cover rounded">
                        </div>
                    @endif
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-green-500 hover:bg-green-50 transition" id="imageDropZone">
                        ...
                        <div id="imagePlaceholder">
                            <p class="text-gray-600 text-lg">📷 Klik untuk ganti foto atau drag & drop</p>
                ...
                <div class="flex gap-4 pt-6">
                    ...
                    <a 
                        href="{{ route('sales_forum.show', $salesForum) }}" 
